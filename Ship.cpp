...
{
    QStringList shipOptions;
    shipOptions<<"ICurStarId"<<"IFullName"<<"Goods"<<"Money"<<"EqList ^{"<<"ArtsList ^{"<<"EqShop ^{"<<"Storage ^{"<<"ShopGoods"<<"ShopGoodsSale"<<"ShopGoodsBuy"<<"IType";
    ...
    while (!line.isNull())
    {
        ...
        switch(shipOptions.indexOf(varname))
        {
        case 0://ICurStarId
            _starId=value.toInt();
        break;
