...
QString bbSeparatedValues(const QItemSelectionModel *selectionModel)
{
	...
	for (const QModelIndex &index : indexes) {
		QString text =
			"[td]" + model->data(previous).toString() + "[/td]";
		// At this point `text` contains the text in one cell
		selectedText.append(text);
		// If you are at the start of the row the row number of the
		// previous index isn't the same.  Text is followed by a row
		// separator, which is a newline.
		if (index.row() != previous.row()) {
			selectedText.append("[/tr]\n[tr]");
		}
		previous = index;
	}
	selectedText.append("[td]" + model->data(previous).toString()
			    + "[/td]");
...
MainWindow::MainWindow(QWidget *parent)
    : QMainWindow(parent), ui(new Ui::MainWindow), tradeModel(&galaxy, this),
      tradeProxyModel(this), eqModel(&galaxy, this), bhModel(&galaxy, this),
      planetsModel(&galaxy, this)
{
	...
	QToolButton *reloadButton = static_cast<QToolButton *>(
		ui->mainToolBar->widgetForAction(ui->actionReload));
	...
	QToolButton *reportButton = static_cast<QToolButton *>(
		ui->mainToolBar->widgetForAction(ui->actionSaveReport));
	...
	ui->mainToolBar->insertWidget(ui->mainToolBar->actions()[3],
				      &_mapScaleSpinBox);
	connect(&_mapScaleSpinBox, SIGNAL(valueChanged(int)), this,
		SLOT(setMapScale(int)));

	_mapFontSpinBox.setMaximum(99);
	_mapFontSpinBox.setSingleStep(1);
	_mapFontSpinBox.setToolTip(tr("Map font size"));
	ui->mainToolBar->insertWidget(ui->mainToolBar->actions()[4],
				      &_mapFontSpinBox);
	connect(&_mapFontSpinBox, SIGNAL(valueChanged(int)), this,
		SLOT(setMapFontSize(int)));
	...
	HierarchicalHeaderView *hv =
		new HierarchicalHeaderView(Qt::Horizontal, ui->tradeTableView);
	...
	planetsHeaderView = new FilterHorizontalHeaderView(
		&planetsProxyModel, ui->planetsTableView);
	...
	eqHeaderView = new FilterHorizontalHeaderView(&eqProxyModel,
						      ui->equipmentTableView);
	...
	ui->equipmentTableView->setColumnWidth(13,
					       tableFontWidth * 48); // Bonus
	// ui->equipmentTableView->resizeRowsToContents();
	ui->equipmentTableView->verticalHeader()->setContextMenuPolicy(
	...
	for (const QString &name : colorNames) {
		...
		connect(act, &QAction::triggered, [this, color]() {
			eqProxyModel.setData(eqProxyModel.index(eqMenuRow, 0),
					     color);
		});
		eqMenu.addAction(act);
	}

	ui->equipmentTableView->verticalHeader()->setDefaultSectionSize(
		tableFontHeight);
	ui->tradeTableView->verticalHeader()->setDefaultSectionSize(
		tableFontHeight);
	ui->planetsTableView->verticalHeader()->setDefaultSectionSize(
		tableFontHeight);
	ui->bhTableView->verticalHeader()->setDefaultSectionSize(
		tableFontHeight);

	ui->equipmentTableView->verticalHeader()->setFixedWidth(tableFontWidth
								* 8);
	ui->planetsTableView->verticalHeader()->setFixedWidth(tableFontWidth
							      * 5);
...
void MainWindow::writeSettings() const
{
	...
	settings.setValue("autoSaveReport",
			  ui->actionAutoSaveReport->isChecked());
...
bool MainWindow::parseDump(const QString &filename)
{
	...
	if (filename.isEmpty()
	    && QFileInfo(_filename).lastModified() == _fileModified) {
		// dump was parsed earlier
		showMessage(tr("dump was parsed earlier, skipping")
			    + _filename);
		return false;
	}
	...
	QString buf = stream.read(500);
	stream.seek(0);
	if (buf.contains(QChar(65533))) {
		stream.setCodec("Windows-1251");
	}
	std::cout << "File encoding: " << stream.codec()->name().toStdString()
		  << ", " << filename.toStdString() << std::endl;
	buf = stream.readAll();
	high_resolution_clock::time_point tReadEnd =
		high_resolution_clock::now();
	auto duration = duration_cast<milliseconds>(tReadEnd - tStart).count();
	string timeTaken = "Reading the file took "
			   + to_string(duration / 1000.0) + " s. ";
	stream.setString(&buf);
	galaxy.parseDump(stream);
	showMessage(
		tr("Parsed %1 stars, %2 planets, %3 black holes, %4 ships and %5 items")
			.arg(galaxy.starCount())
			.arg(galaxy.planetCount())
			.arg(galaxy.blackHoleCount())
			.arg(galaxy.shipCount())
			.arg(galaxy.equipmentCount()),
		5000);
	high_resolution_clock::time_point tParseEnd =
		high_resolution_clock::now();
	...
	// int
	// sectionSize=ui->equipmentTableView->verticalHeader()->sectionSize(0);
	...
	high_resolution_clock::time_point tModelUpdateEnd =
		high_resolution_clock::now();
	duration = duration_cast<milliseconds>(tModelUpdateEnd - tParseEnd)
			   .count();
	timeTaken += "Model update - " + to_string(duration / 1000.0) + " s. ";

	updateMap();
	high_resolution_clock::time_point tMapEnd =
		high_resolution_clock::now();
	duration =
		duration_cast<milliseconds>(tMapEnd - tModelUpdateEnd).count();
...
bool MainWindow::openDump()
{
	QString fileName = QFileDialog::getOpenFileName(
		this, tr("Open Dump File"), rangersDir + "/save/",
...
void MainWindow::showAbout()
{
	QMessageBox::about(
		this, "SRHDDumpReader",
		QString("SRHDDumpReader v. %1\nAuthor: Mykola Dimura\n"
			"Parse and analyse the galaxy dumps from "
			"\"Space Rangers HD: A War Apart\" video game\n"
...
void MainWindow::saveReport()
{
	...
	QString filename =
		fileInfo.path() + '/' + fileInfo.completeBaseName() + ".report";
	...
	for (int row = 0; row < tradeProxyModel.rowCount(); row++) {
		double dist =
			tradeProxyModel.data(tradeProxyModel.index(row, 2))
				.toInt();
		int size = tradeProxyModel.data(tradeProxyModel.index(row, 27))
				   .toInt();
		QString name =
			tradeProxyModel.data(tradeProxyModel.index(row, 0))
				.toString();
		QString star =
			tradeProxyModel.data(tradeProxyModel.index(row, 1))
				.toString();
	...
	for (const QString &fileName : eqReportPresets) {
		...
		for (int proxyRow = 0; proxyRow < eqProxyModel.rowCount();
		     ++proxyRow) {
...
void MainWindow::saveAllReports()
{
	...
	if (!ofile.open(QIODevice::WriteOnly | QIODevice::Text)) {
		showMessage(tr("Could not create the summary report file ")
			    + filename);
		return;
	}
	...
	for (const QString &dumpFileName : dumpFileList) {
		...
		out << QFileInfo(dumpFileName).baseName() + "\t"
				+ reportSummary(false) + '\n';
	}
...
void MainWindow::loadNextDump()
{
	if (currentDumpIndex < 0
	    || currentDumpIndex >= dumpFileList.size() - 1) {
		return;
	}
	parseDump(dumpFileList[++currentDumpIndex]);
	updateDumpArrows();
...
{
	eqMenuRow =
		ui->equipmentTableView->verticalHeader()->logicalIndexAt(pos);
	eqMenu.popup(ui->equipmentTableView->verticalHeader()
			     ->viewport()
			     ->mapToGlobal(pos));
}
...
void MainWindow::savePreset(const QVariantMap &preset,
...
void MainWindow::loadPresets()
{
	...
	for (const QString &fileName : plDir.entryList(QDir::Files)) {
		...
		planetsHeaderView->addPreset(preset,
...
void MainWindow::saveMap()
{
	if (mapWidth > 10) {
		QFileInfo fileInfo(_filename);
		QString filename = fileInfo.path() + '/'
				   + fileInfo.completeBaseName() + "_map.png";
...
QVariantMap MainWindow::loadPreset(const QString &fileName) const
{
	QFile file(fileName);
	if (!file.open(QIODevice::ReadOnly | QIODevice::Text)) {
		showMessage(tr("Unable to open file ") + file.errorString()
			    + " " + file.fileName());
		return QVariantMap();
	}
	QJsonDocument doc = QJsonDocument::fromJson(file.readAll());
...
QMap<QString, MainWindow::Scorer>
MainWindow::readScorers(const QString &filename)
{
...
bool rangersWindowActive()
{
	...
	HANDLE PID =
		OpenProcess(PROCESS_QUERY_INFORMATION | PROCESS_VM_READ, FALSE,
			    windowProcessId /* This is the PID, you can find one
					       from windows task manager */
		);
	if (PID) {
		TCHAR Buffer[MAX_PATH];
		if (GetModuleFileNameEx(PID, 0, Buffer, MAX_PATH)) {
			// At this point, buffer contains the full path to the
			// executable
			std::wstring wstr(Buffer);
			std::string str(wstr.begin(), wstr.end());
			if (QFileInfo(QString::fromStdString(str)).baseName()
			    == "Rangers") {
				return true;
			}
		} else {
...
bool MainWindow::simulateInput(const std::string &str) const
{
	...
	for (char c : str) {
		if (std::islower(c)) {
			if (c == 'e') {
				...
				inp.ki.dwFlags =
					0
					| KEYEVENTF_SCANCODE; // 0 for key press
				ip.push_back(inp);
				inp.ki.dwFlags =
	...
	if (!rangersWindowActive()) {
		showMessage(tr(
			"Rangers are not in focus, aborting input simulation."));
		return false;
	}
...
void MainWindow::saveDumpWin()
{
	...
	if (!rangersWindowActive()) {
		showMessage(
			tr("Rangers are not in focus, aborting QuickDump."));
		return;
	}
	...
	for (int t = 0; t < 30000; t += dt) {
		...
		if (size && size == oldSize) {
			...
			QFile::rename(cyrFilename.left(cyrFilename.length() - 4)
					      + ".sav",
				      _filename.left(_filename.length() - 4)
					      + ".sav");
			parseDump();
			return;
		}
	...
	showMessage(
		tr("Could not save the dump, timeot reached. Not parsing."));
}
...
QImage MainWindow::currentScreen(float kx, float ky, float kw, float kh)
{
	...
	const QString rangersDir = QStandardPaths::locate(
		QStandardPaths::DocumentsLocation, "SpaceRangersHD",
	...
	if (screenFilename.isEmpty()) {
		cerr << string("Could not take a screenshot at time: ")
				+ to_string(t)
		     << endl;
		return QImage();
	}
	...
	currentImage = currentImage.copy(
		currentImage.width() * kx, currentImage.height() * ky,
...
void MainWindow::generateGalaxies()
{
	...
	using namespace std::chrono;
	std::string filename =
		QFile::encodeName(rangersDir + "/SRHDDumpReader_generation.log")
			.toStdString();
	ofstream logfile(filename, std::ofstream::out | std::ofstream::app);
	if (!logfile.good()) {
		showMessage(tr("Could not open the log file ")
	...
	if (originalMainMenu.height() == 0) {
		showMessage(tr(
			"could not get reference screenshot for the main menu. Generation stopped."));
		return;
	}
	...
	for (int i = 0;; i++) {
		high_resolution_clock::time_point iterationStart =
			high_resolution_clock::now();
		...
		if (currentMainMenu != originalMainMenu) {
			...
			showMessage(tr(
				"Main menu is expected, but does not match. Generation stopped. "));
			return;
		}
		...
		responsiveSleep(shortSleep * 8);
		// Enter (continue to generation if extra settings opened,
		// ignored if it is already generating)
		if (!simulateInput("e")) {
			return;
		}
		// Wait until the galaxy is ready
		high_resolution_clock::time_point tGenerationStarted =
			high_resolution_clock::now();
		QImage currentGenerationFinished;
		if (i == 0) { // first run
			responsiveSleep(maxGenerationTime);
			originalGenerationFinished =
				currentScreen(0.45, 0.925, 0.1, 0.05);
			originalGenerationFinished.save(
				"originalGenerationFinished.png");
		} else {
			responsiveSleep(generationTime);
			const int dt = 3000;
			int t;
			numGenChecks[i % historyLength] = 0;
			for (t = generationTime; t < maxGenerationTime;
			     t += dt) {
				currentGenerationFinished =
					currentScreen(0.45, 0.925, 0.1, 0.05);
			...
			if (t >= maxGenerationTime) {
				currentGenerationFinished.save(
					"failedGenerationFinished.png");
				showMessage(tr(
					"Galaxy_generation_finished is expected, but does not match. Generation stopped"));
				return;
			}
			high_resolution_clock::time_point tGenerationFinished =
				high_resolution_clock::now();
			auto curGenerationTime = duration_cast<milliseconds>(
							 tGenerationFinished
							 - tGenerationStarted)
							 .count();
			// realGenTimes[i%10]=generationTime;
			std::cout << "Last galaxy generation took: "
				  << curGenerationTime / 1000.0 << "s. ";
			int minChecks = *std::min_element(numGenChecks.begin(),
							  numGenChecks.end());
			std::cout << "Succeeded after "
				  << numGenChecks[i % historyLength]
				  << " checks. ";
			...
			std::cout << "Trying next: " << generationTime / 1000.0
				  << "s." << std::endl;
		}
		...
		if (isUseless(_reportSummary, minRowsPreset)) { // useless save
			...
			QFile::rename(prefix + ".txt",
				      prefix + timestamp + ".txt");
			QFile::rename(prefix + ".sav",
				      prefix + timestamp + ".sav_");
			QFile::rename(prefix + ".report",
				      prefix + timestamp + ".report");
			QFile::rename(prefix + "_map.png",
				      prefix + timestamp + "_map.png");
		}
		...
		int iterationTime =
			duration_cast<seconds>(high_resolution_clock::now()
					       - iterationStart)
				.count();
		std::string logoutstr = timestamp.toStdString() + ": Iteration "
					+ std::to_string(i) + " finished in "
					+ to_string(iterationTime) + " s. ";
...
void MainWindow::Scorer::addPreset(const QString presetName, double weight,
				   bool isBool, bool depthPenalize)
{
...
QStringList supportedColors()
{
	...
	if (!file.open(QIODevice::ReadOnly | QIODevice::Text)) {
		std::cerr << "could not open selection_colors.json"
			  << std::endl;
		...
		colorNames =
			QJsonDocument::fromJson(data).array().toVariantList();
	}
	QStringList strs;
